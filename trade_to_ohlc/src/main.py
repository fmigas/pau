...
def transform_trade_to_ohlcv(
    ...
    Returns:
        None
    """
    logger.info("Starting the trade to OHLCV transformation")
    logger.info(f"Reading from the topic: {kafka_input_topic}")
    logger.info(f"Writing to the topic: {kafka_output_topic}")
    logger.info(f"Consumer group: {kafka_consumer_group}")
    logger.info(f"Reading from the beginning: {read_from_beginning}")
